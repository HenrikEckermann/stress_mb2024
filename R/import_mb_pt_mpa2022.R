...
rn <- as.data.frame(rowData(tse_s))$clade_name %>% map_chr(~unname(.x))
rownames(tse_s) <- rn
tree <- read.tree(here("data/processed_by_turku/mpa_vOct22_CHOCOPhlAnSGB_202212.nwk"))
# the tree tip labels are the SGB ids. In order to map we need the following file:
file <- here("data/processed_by_turku/mpa_vOct22_CHOCOPhlAnSGB_202212_species.txt")
...
species_names$sgb_id[!tree$tip.label %in% species_names$sgb_id]
head(species_names)
    ...
    str_detect(colnames(tse_s), "Ep32"), 32, ifelse(
      str_detect(colnames(tse_s), "Ep18"), 18, ifelse(
        str_detect(colnames(tse_s), "E6w"), 6, ifelse(
          str_detect(colnames(tse_s), "E8MO"), 32, NA))))))
